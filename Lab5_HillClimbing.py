...
def is_valid_permutation(state):
    """Checks if the state is a valid permutation of 0, 1, 2, and 3."""
    return sorted(list(state)) == list(range(4))

def compute_cost(state):
    """Computes the number of diagonal conflicts for a 4x4 board."""
    attacks = 0
    n = len(state)
    for i in range(n):
